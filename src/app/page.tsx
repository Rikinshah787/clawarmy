...
*Generated by [ClawArmy](https://github.com/Rikinshah787/ClawArmy) on ${timestamp}*
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between relative p-4 glass rounded-2xl targeting-reticle targeting-reticle-tl targeting-reticle-tr targeting-reticle-bl targeting-reticle-br">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={async () => {
                const hubScript = `@echo off
TITLE ClawArmy Universal Sync Hub
echo ClawArmy: Universal Sync Hub [ACTIVE]
echo =====================================
echo This hub will keep your local project in sync with the ClawArmy Marketplace.
echo Target Directory: %CD%\\agents
echo.
echo [1] Manual Sync Now
echo [2] Auto-Sync Loop (Every 5 mins)
set /p opt="Select Operation Mode: "
...
goto loop`;
                ...
                const blob = new Blob([hubScript], { type: "text/plain" });
                const url = URL.createObjectURL(blob);
                const link = document.createElement("a");
                link.href = url;
                link.download = "clawarmy-sync.bat";
                link.click();
                URL.revokeObjectURL(url);
              }}
              className="px-4 py-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-xs font-bold hover:bg-emerald-500/20 transition-all flex items-center gap-2 glow tech-font"
              title="Download persistent sync hub for your project"
            >
              <span>🔄</span>
              HUB_SYNC_CLIENT
            </button>
